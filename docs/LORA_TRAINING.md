...
1. **Ollama models** — `make pull-models` pulls code/utility tiers plus `llama3:8b` for biology LoRAs. Enabled packs also trigger background pulls via `models_to_ensure`.
2. **Python training stack** — `make deps` (or `make gui-install`) creates `.venv-lora` and installs `requirements-lora.txt`. The hub uses that venv for training jobs automatically.
3. **CUDA** — strongly recommended; CPU training is experimental and slow.
4. **Hugging Face access** for gated bases (Llama 3) — `huggingface-cli login` or set `HF_TOKEN`.

## Training wizard (Desktop)

Open **Model library** (⇧⌘M) → **Train LoRA** tab.
...
- Assign the composed tag via **Settings → Specialist model overrides** or `/switch-provider`.
