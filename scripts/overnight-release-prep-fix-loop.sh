#!/usr/bin/env bash
# Walk-away release-prep fix loop: Ollama, optional model pre-pull, caffeinate, fix loop in tmux.
# Invoked via: make overnight-release-prep-fix-loop
set -euo pipefail
...
SESSION="${NJ_OVERNIGHT_SESSION:-nj-release-prep-fix-loop}"
STAMP="$(date +%Y%m%d-%H%M)"
LOG_DIR="${NJ_OVERNIGHT_LOG_DIR:-${HOME}}"
LOG_FILE="${NJ_OVERNIGHT_LOG:-${LOG_DIR}/nj-overnight-fix-loop-${STAMP}.log}"
...
require_ollama() {
  ...
  echo "FAIL: Ollama is required for overnight release-prep fix loop." >&2
...
build_tmux_inner_command() {
  ...
  inner+=" IN_TMUX=0 && '${ROOT}/scripts/overnight-release-prep-fix-loop.sh'"
  printf '%s' "${inner}"
}
...
run_overnight() {
  local target="release-prep-fix-loop"
  local -a make_args=()
  local line
  ...
  {
    echo ">>> Overnight fix loop started $(date -u '+%Y-%m-%d %H:%M:%S UTC')"
    echo ">>> Target: make ${target}"
    echo ">>> Log: ${LOG_FILE}"
    echo ">>> Branch: ${FIX_BRANCH:-release-prep/fix-<auto>}"
  ...
  echo ""
  if [[ "${rc}" -eq 0 ]]; then
    echo "=== Overnight fix loop PASS ==="
  else
    echo "=== Overnight fix loop FAIL (exit ${rc}) ===" >&2
  fi
  echo "Log: ${LOG_FILE}"
  echo "Review latest: ls -lt ${ROOT}/docs/testing/release-prep-fix-loop-*.md ${ROOT}/docs/testing/release-prep-*.md 2>/dev/null | head -5"
  return "${rc}"
}

if [[ -z "${NJ_OVERNIGHT_INNER:-}" ]] && [[ "${IN_TMUX:-1}" != "0" ]] && [[ -z "${TMUX:-}" ]]; then
  chmod +x "${ROOT}/scripts/overnight-release-prep-fix-loop.sh"
  ...
  if ! command -v tmux >/dev/null 2>&1; then
    ...
    if tmux new-session -d -s "${SESSION}" "${inner}" 2>/dev/null; then
      echo "Started overnight release-prep fix loop in tmux session: ${SESSION}"
