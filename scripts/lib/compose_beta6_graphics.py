#!/usr/bin/env python3
"""Render v1.2.0-beta.6 release article cover (1200×627)."""
...
def render_article_cover(out: Path, gallery: Path | None = None) -> None:
    ...
    # Version badge
    draw.rounded_rectangle((lx, 36, lx + 248, 64), radius=6, fill=(20, 28, 40), outline=CYAN, width=2)
    draw.text((lx + 124, 50), "v1.2.0-beta.6 · OPEN BETA", fill=CYAN, font=font(True, 11), anchor="mm")

    hy = 88
    draw.text((lx, hy), "A MEMORY OF", fill=TEXT, font=font(True, 36), anchor="lt")
    draw.text((lx, hy + 42), "ITS OWN CODE", fill=CYAN, font=font(True, 40), anchor="lt")

    sy = hy + 110
    fsub = font(False, 15)
    for line in wrap(
        draw,
        "Native knowledge graph. Model Arena. Stream subscriptions. Homebrew. The workstation that remembers how your repo connects.",
        fsub,
        lw,
    ):
    ...
    bullets = [
        ("Repository knowledge graph", CYAN),
        ("Model Arena + usage telemetry", AMBER),
        ("MQTT/Kafka stream triggers", GREEN),
    ]
    ...
    row_h = 108
    features = [
        ("KNOWLEDGE GRAPH", "nodes · paths · explain", CYAN),
        ("MODEL ARENA", "head-to-head matches", AMBER),
        ("STREAM SUBS", "MQTT · Kafka → runbooks", GREEN),
        ("BONSAI 27B", "PrismML · mmproj", PINK),
        ("ROOM CHAT", "ephemeral LAN rooms", TEAL),
        ("HOMEBREW", "macOS cask · Linux formula", AMBER),
        ("PACK V2", "CAD · AWS · software-dev", PINK),
        ("PREVIEW FIXES", "images · pack toolbar chips", GREEN),
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Render beta.6 release article cover")
    parser.add_argument(
        "variant",
        choices=["article"],
    ...
    root = args.root
    assets = root / "campaigns" / "beta26" / "creatives"
    assets.mkdir(parents=True, exist_ok=True)
    gallery = root / "docs" / "media" / "gallery" / "ads"

    if args.variant == "article":
        render_article_cover(
            assets / "neural-junkie-beta6-1200.png",
            gallery / "neural-junkie-beta6-1200.png",
        )
