...
class HubConfigTest(unittest.TestCase):
    def test_env_or_automation_prefers_env(self) -> None:
        with mock.patch.dict(os.environ, {"NJ_DELIVERABLE_JUDGE_PROVIDER": "ollama"}, clear=False):
            with mock.patch("lib.hub_config.automation_config", return_value={"deliverable_judge_provider": "gemini"}):
                self.assertEqual(env_or_automation("NJ_DELIVERABLE_JUDGE_PROVIDER", "deliverable_judge_provider", "gemini"), "ollama")
