...
def start_regression_hub(repo_root: Path) -> subprocess.Popen[bytes] | None:
...
def restart_regression_hub(repo_root: Path, hub_url: str, timeout_s: float = 120.0) -> bool:
    stop_hub(repo_root)
    if start_regression_hub(repo_root) is None:
        return False
