#!/usr/bin/env bash
# Compose neural-junkie-semantic-turn-routing-1200.png — LinkedIn article cover (1200×627).
# Usage: ./scripts/compose-semantic-turn-routing-article.sh
#
...
OUT="$ROOT/campaigns/semantic-turn-routing/creatives/neural-junkie-semantic-turn-routing-1200.png"
GALLERY="$ROOT/docs/media/gallery/ads/neural-junkie-semantic-turn-routing-1200.png"
...
def fit_cover(src: Path) -> Image.Image | None:
    ...
    if im.size == (W, H):
        return im
    # Center-crop to 1200×627 aspect, then resize.
    tw, th = W, H
...
if existing is not None and OUT.stat().st_size > 100_000:
    # Keep the illustrated cover; ensure exact LinkedIn dimensions.
    canvas = existing
else:
    ...
    head_y = 68
    draw.text((MARGIN, head_y), "ONE DECISION PER TURN.", fill=(255, 255, 255), font=font(True, 30), anchor="lt")
    draw.text((MARGIN, head_y + 38), "MEANING, NOT PHRASES.", fill=TEAL, font=font(True, 32), anchor="lt")

    sub_y = head_y + 84
    for line in wrap(
        draw,
        "Local classifier describes meaning · deterministic policy grants permission",
        font(False, 14),
        CONTENT_W,
    ):
    ...
    nodes = [
        ("Desktop", "explicit mode", TEAL),
        ("Hub ingress", "TurnFeatures", GREEN),
        ("Local classify", "JSON schema", BLUE),
        ("TurnDecision", "policy stamp", ACCENT),
    ]
    node_w = 220
    node_h = 78
    ...
    items = ["action", "recipient", "retrieval", "mutation", "evidence", "Ask/Plan safe"]
    ...
    draw.text(
        ...
        "NEURAL JUNKIE · SEMANTIC TURN ROUTING",
        fill=(255, 255, 255),
        font=font(True, 13),
        anchor="mm",
    )
...
# If we kept the illustrated cover, still write exact size; else canvas is drawn.
