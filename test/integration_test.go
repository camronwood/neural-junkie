...
// TestHelperAgentWorkflow tests complete helper agent workflow
func TestHelperAgentWorkflow(t *testing.T) {
	...
	// Clean up helper agent cache after test
	cleanupHelperAgentCache(t, "ProgrammingHelper")

	// Create test knowledge files
	knowledgeFiles := map[string]string{
		"programming.md": `# Programming Help

This document provides programming assistance.
...
This document provides debugging assistance.
...
- Logging`,
	...
	config := &agent.HelperAgentConfig{
		...
		Description:   "A programming assistance helper",
		Expertise:     []string{"programming", "debugging", "go"},
		Keywords:      []string{"help", "assist", "debug", "programming"},
	...
	// Create helper agent
	helperAgent, err := agent.NewHelperAgent(config, mockAI, h)
	if err != nil {
		t.Fatalf("Expected helper agent creation to succeed, got error: %v", err)
	}

	// Register helper agent with hub
	err = h.RegisterAgent(&helperAgent.Info)
	if err != nil {
		t.Fatalf("Expected helper agent registration to succeed, got error: %v", err)
	}
	...
	// Create test channel
	h.CreateChannel("helper-test", "Helper test channel", "test-project")

	// Start agent
	ctx := context.Background()
	channel := "helper-test"
	err = helperAgent.Start(ctx, channel)
	if err != nil {
		t.Fatalf("Expected helper agent to start, got error: %v", err)
	}

	// Give agent time to start
	time.Sleep(100 * time.Millisecond)

	// Test helper questions
	userInfo := protocol.AgentInfo{
		ID:   "user-123",
		Name: "TestUser",
		Type: protocol.AgentTypeGeneral,
	...
	// Test 1: Programming help question
	programmingQuestion := protocol.NewMessage(
		protocol.MessageTypeQuestion,
		channel,
		userInfo,
		"@ProgrammingHelper Can you help me with Go programming?",
	)
	programmingQuestion.Mentions = []string{helperAgent.Info.ID}

	err = h.SendMessage(programmingQuestion)
	if err != nil {
		t.Fatalf("Expected message sending to succeed, got error: %v", err)
	}
	...
	// Test 2: Debugging help question
	debuggingQuestion := protocol.NewMessage(
		protocol.MessageTypeQuestion,
		channel,
		userInfo,
		"@ProgrammingHelper I have a bug in my code, can you help me debug it?",
	)
	debuggingQuestion.Mentions = []string{helperAgent.Info.ID}

	err = h.SendMessage(debuggingQuestion)
	if err != nil {
		t.Fatalf("Expected message sending to succeed, got error: %v", err)
	}
	...
	// Check if helper agent responded
	messages, err := h.GetMessages(channel, 10)
	if err != nil {
		t.Fatalf("Expected to get messages, got error: %v", err)
	}
	...
	for _, msg := range messages {
		if msg.From.Name == "ProgrammingHelper" && msg.Type == protocol.MessageTypeChat {
	...
	if !helperResponded {
		t.Error("Expected helper agent to respond to questions")
	}
