...
// Host-side only — consent + LocationStore cannot live in the pack sidecar.
...
func AttachLocateToolWithStore(mcpServer *server.MCPServer, store *LocationStore) {
	if mcpServer == nil {
		return
	}
...
func (t *locateTool) locationStore() *LocationStore {
	if t.store != nil {
		return t.store
	}
...
func (t *locateTool) handleLocate(ctx context.Context, request mcpgo.CallToolRequest) (*mcpgo.CallToolResult, error) {
	...
	var out *LocateRequest
	var waitErr error
	go func() {
		out, waitErr = store.WaitLocate(req.ID, LocateRequestTTL)
		close(done)
	}()
	select {
	case <-ctx.Done():
