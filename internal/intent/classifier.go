...
const semanticClassifierPromptPrefix = `You are the semantic understanding stage of an agent runtime.
...
const semanticClassifierPromptMid = `",
  "recipient_type": "`

const semanticClassifierPromptSuffix = `",
...
- continue advances one pending action. When interaction is continuation, requested_action must be continue and continuation_target must copy pending_action_id. Soft follow-ups ("and what about the second option?") without pending_action_id are question/answer — never continue or ask_user.
- factual questions about which package/file declares a dependency or where something lives in this repo/codebase → inspect with codebase retrieval and reason_codes including repo_fact — never answer-only.
...
- When open_artifact_id / open_artifact_renderer is set in features, prefer artifact for turns that change that open canvas (add/fill text, mermaid, or images on that page); include prior_reference. Pure questions about the open page stay answer — never artifact. Examples: "did you update the canvas?", "is this canvas up to date?", "what's on the canvas?", "why did you name it X?" → action=answer, mutation=none, reason_codes including canvas_meta_question (or canvas_title_question for why-named / title asks). Renaming/retitling the open canvas is artifact — never canvas_meta_question or answer-only.
...
- use stable reason codes such as startup_failure, build_failure, runtime_failure, explicit_continuation, correction, advisory_question, durable_artifact, blank_canvas, workspace_report, canvas_meta_question, canvas_title_question, project_overview, git_inspect, or repo_fact.
- check git status/log/diff/history asks → inspect with reason_codes including git_inspect — never run or edit.
- use ask_user when a required target is genuinely ambiguous (missing file/path/entity the user must choose). Never stamp ask_user for thanks/closure, soft acknowledgements, topic switches, presence checks, or ordinary chat questions that can be answered without a user choice.`

// semanticClassifierPrompt returns the classify system prompt with dynamic domain/recipient enums.
func semanticClassifierPrompt() string {
	reg := CurrentOntology()
	return semanticClassifierPromptPrefix +
		strings.Join(reg.DomainEnum(), "|") +
		semanticClassifierPromptMid +
		strings.Join(reg.RecipientEnum(), "|") +
		semanticClassifierPromptSuffix
}

// SemanticIntentSchema is the structured-output JSON schema for the classifier.
// Prefer SemanticIntentSchemaJSON() so pack-extended ontologies are reflected.
var SemanticIntentSchema = SemanticIntentSchemaJSON()

// SemanticIntentSchemaJSON builds the classifier JSON schema from the active ontology.
func SemanticIntentSchemaJSON() json.RawMessage {
	reg := CurrentOntology()
	domainEnum, _ := json.Marshal(reg.DomainEnum())
	recipientEnum, _ := json.Marshal(reg.RecipientEnum())
	raw := `{
  "type": "object",
  ...
  "properties": {
    ...
    "domain": {"type": "string", "enum": ` + string(domainEnum) + `},
    "recipient_type": {"type": "string", "enum": ` + string(recipientEnum) + `},
