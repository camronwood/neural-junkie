...
// SidecarBaseURL resolves the AWS pack sidecar base URL when wired by the hub server.
var SidecarBaseURL func() string

// Client calls the AWS pack boto3 sidecar.
type Client struct {
	BaseURL func() string
	HTTP    *http.Client
}
...
func (c *Client) resolve() (*Client, error) {
	...
	return nil, fmt.Errorf("aws sidecar not configured")
}
...
func (c *Client) base() (string, error) {
	...
	if base == "" {
		return "", fmt.Errorf("aws sidecar not running (enable AWS pack and run setup-aws-sidecar.sh)")
	}
	return base, nil
}
...
func (c *Client) Status(ctx context.Context) (map[string]any, error) {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, base+"/api/aws/status", nil)
	if err != nil {
		return nil, err
	}
	client, _ := c.resolve()
	...
	if resp.StatusCode >= 400 {
		return nil, fmt.Errorf("aws sidecar status: %s", resp.Status)
	}
	return out, nil
}
