...
func TestListPackUpdatesDetectsNewerCatalogVersion(t *testing.T) {
	...
	if updates[0].InstalledVersion == "" || updates[0].LatestVersion != "2.2.0" {
	...
	if row == nil || !row.UpdateAvailable || row.Version != "2.2.0" {
...
func TestUpgradePackPreservesEnabled(t *testing.T) {
	...
	upgradedZipURL := serveTestPackZipAtVersion(t, PackMusicCreation, "2.2.0")
	packs.InvalidateCatalogCache()
	body, _ := json.Marshal(&packs.Catalog{
		Version: 1,
		Packs: []packs.CatalogEntry{{
			ID: PackMusicCreation, Version: "2.2.0", Title: "Music", DownloadURL: upgradedZipURL,
		}},
	})
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
	...
	if ver != "2.2.0" {
		t.Fatalf("installed version = %q, want 2.2.0", ver)
	}
}
